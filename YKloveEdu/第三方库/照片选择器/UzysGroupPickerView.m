...
- (void)setupLayout
{
    ...
    self.frame = CGRectMake(0, - [UIScreen mainScreen].bounds.size.height, kScreenWidth, [UIScreen mainScreen].bounds.size.height);;
...
- (void)setupTableView
{
    self.tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, NavigationHeight, kScreenWidth, self.bounds.size.height -NavigationHeight) style:UITableViewStylePlain];
...
- (void)show
{
    [UIView animateWithDuration:0.25f delay:0.f options:UIViewAnimationOptionCurveEaseInOut|UIViewAnimationOptionBeginFromCurrentState animations:^{
        self.frame = CGRectMake(0, BounceAnimationPixel , kScreenWidth, [UIScreen mainScreen].bounds.size.height);
            ...
            [UIView animateWithDuration:0.15f delay:0.f options:UIViewAnimationOptionCurveEaseOut|UIViewAnimationOptionBeginFromCurrentState animations:^{
                    self.frame = CGRectMake(0, 0 , kScreenWidth, [UIScreen mainScreen].bounds.size.height);
            } completion:^(BOOL finished) {
            }];
    }];
    self.isOpen = YES;
