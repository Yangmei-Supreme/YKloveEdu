...
//  XLScrollDemo
//
//  Created by 杨梅 on 15/1/12.
//  Copyright (c) 2015年 May Young All rights reserved.
//
...
-(void)addScroll1{
    ...
    self.view2 =[[UIView alloc]initWithFrame:CGRectMake(_x0,CGRectGetMaxY(button0.frame)-(self.xl_buttonToSlider?self.xl_buttonToSlider:2), size0.width, self.xl_sliderHeight?self.xl_sliderHeight:2)];
    self.view2.backgroundColor =self.xl_sliderColor?self.xl_sliderColor:[UIColor colorWithRed:20 / 255.0 green:170 / 255.0 blue:0 / 255.0 alpha:1];
    if (self.xl_isSliderCorner) {
        [self.view2.layer setCornerRadius:self.xl_sliderCorner?self.xl_sliderCorner:0];
    }
...
-(void)addScroll2{
    ...
    for (int i =0; i<self.xl_buttonNames.count; i++) {
        ...
        if (!self.xl_views) {
            ...
            temp.frame =(CGRect){{screen_width*i, 0},kScreenWidth, self.scroll2.frame.size.height - 65};
            [self.scroll2 addSubview:temp];
        }
    }
